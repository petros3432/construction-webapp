...
public interface RepairRepository extends JpaRepository<Repair,Long> {
    ...
    @Query(value="SELECT * FROM Repair,Owner WHERE Repair.Owner_id=Admin.Owner_id AND Owner.AFM=(:afm) AND Repair.scheduledDate=(:startDate)", nativeQuery = true)
    Optional<List<Repair>> advSearchAfmDate(String afm, LocalDate startDate);

    @Query(value="SELECT * FROM Repair,Owner WHERE Repair.Owner_id=Admin.Owner_id AND Owner.AFM=(:afm) AND (Repair.scheduledDate=>(:startDate) AND Repair.scheduledDate<=(:endDate))", nativeQuery = true)
    Optional<List<Repair>> advSearchAfmDateBandwidth(String afm, LocalDate startDate, LocalDate endDate);

    @Query(value="SELECT * FROM Repair,Owner WHERE Repair.Owner_id=Admin.Owner_id AND Repair.scheduledDate=(:startDate)", nativeQuery = true)
    Optional<List<Repair>> advSearchDate(LocalDate startDate);

    @Query(value="SELECT * FROM Repair,Owner WHERE Repair.Owner_id=Admin.Owner_id AND (Repair.scheduledDate=>(:startDate) AND Repair.scheduledDate<=(:endDate))", nativeQuery = true)
